...
resource "null_resource" "cloud_init_k3s_server_user_data_file_pve2" {
  triggers = {
...
resource "proxmox_vm_qemu" "k3s_server" {
  depends_on = [null_resource.cloud_init_k3s_server_user_data_file_pve2]
...
locals {
  nuc1_agent_network_mac_addresses = {
    0 = "E0:B1:D5:8C:79:3E"
    1 = "1E:AA:66:A9:52:9D"
    2 = "8E:FB:06:C5:89:72"
    3 = "52:2E:F5:A0:35:76"
...
EOF
  ...
  network {
    ...
    tag = 10
    firewall = false
    macaddr = local.nuc1_agent_network_mac_addresses[count.index]
  }
...
resource "null_resource" "cloud_init_k3s_agent_user_data_file_pve2" {
  count = var.k3s_agent_count["pve2"]
  triggers = {
    user_data_file = filemd5("${path.module}/files/k3s_agent_user_data.yml")
    k3s_token : random_password.k3s_token.result,
  }
...
locals {
  pve2_agent_network_mac_addresses = {
    0 = "8E:FB:06:C5:89:71"
    1 = "8E:FB:06:C5:89:72"
    2 = "8E:FB:06:C5:89:73"
    3 = "8E:FB:06:C5:89:74"
...
resource "proxmox_vm_qemu" "k3s_agent_pve2" {
  depends_on = [null_resource.cloud_init_k3s_agent_user_data_file_pve2, proxmox_vm_qemu.k3s_server]
  count = var.k3s_agent_count["pve2"]

  name = "k3s-agent-pve2-${count.index + 1}"
  desc = "K3S Agent"
  tags = "k3s_agent"
  target_node = "pve2"
  os_type = "cloud-init"
  clone = "ubuntu-2204-cloudinit-template-pve2"
  onboot = true
  scsihw = "virtio-scsi-pci"
  ciuser = "provisioner"
  ...
  cores = var.k3s_agent_core_count["pve2"]
  sockets = 1
  memory = var.k3s_agent_memory_amount["pve2"]
  agent = 1
...
EOF
  ...
  disk {
    type = "scsi"
    storage = "local"
    size = var.k3s_agent_disk_size["pve2"]
    backup = false
  }
  ...
  network {
    ...
    tag = 10
    firewall = false
    macaddr = local.pve2_agent_network_mac_addresses[count.index]
  }

  ipconfig0 = "ip=192.168.10.6${count.index +1}/24,gw=192.168.10.1"
